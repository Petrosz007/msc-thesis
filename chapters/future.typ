...
In their paper, Hierons has shown that because BVA only generates single points on boundaries, geometric shifts in boundaries can lead to accidental correctness @coincidentalCorrecttness. In other words, there are some incorrect implementations, which cannot be caught with BVA.
...
@coincidental-correctness-example shows the two functions: $f(x) = 0$ in green and $f(x) = x$ in orange. In this scenario the test points A, B, C, and D show that both functions behave in the same way. In contract, point E and F could show the bug, because they are both under the slope of $f(x) = x$.
...
In the second implementation, if we replace that `age >= 45` with any number in $[46,59]$ the original tests in @paid-vacation-days-final-reduced-test-cases still all pass. However, we know for sure that we've made an error, because we've replaced that number.
