...
If this problem would be one dimentional, as in, we only had simple intervals, we could use the $O(n log n)$ solution proposed in @stackexchangeMinimumPointCover. However, this algorithm assumes that the intervals can be sorted in increasing order by their hi endpoints. Because we are dealing with NTuples and we could have $n$ intervals for each interval, we can't use this algorithm, because we can't define a partial ordering on it that'd work in this case.
...
We calculate the weights of all the edges. We join the edge with the smallest weight (the one that loses the least edges). After a join, all the edge weigths have to be recalculated, because subsequent joins could affect the edges differently, than the number we calculated.
