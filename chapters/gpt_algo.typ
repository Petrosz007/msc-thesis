...
The output values must be generated by the test designer by hand. GPT only generates the inputs for the test cases. If it could generate the outputs as well, that would mean that it could generate the correct implementation as well, which it can't.
